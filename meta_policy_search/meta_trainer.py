...
class Trainer(object):
    ...
    def __init__(
        ...
        os.environ["OMP_NUM_THREADS"] = str(CPS)
        os.environ["MKL_NUM_THREADS"] = str(CPS)
        os.environ["GOTO_NUM_THREADS"] = str(CPS)
        os.environ["openmp"] = 'True'
        if sess is None:
            config = tf.ConfigProto(
                        device_count={"CPU":CPS},
                        intra_op_parallelism_threads=CPS,
                        inter_op_parallelism_threads=CPS,
        ...
        with self.sess.as_default() as sess:
            ...
            for itr in range(self.start_itr, self.n_itr):
                ...
                for step in range(self.num_inner_grad_steps+1):
                    ...
                    # TODO: process samples of each task with different baseline could be better?
